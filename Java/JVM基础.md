...
>
> Java 堆在虚拟机启动的时候就被创建，它存储了被自动内存管理系统（automatic storage management system），也就是常说的 garbage collector（垃圾收集器）所管理的各种对象，这些受管理的对象无需也无法显式地销毁。Java 虚拟机并未假设采用何种具体的技术去实现自动内存管理系统，虚拟机的实现者可以根据系统的实际需要来选择自动内存管理工具。Java 堆的容量可以是固定的，也可以随着程序执行的需求动态地扩展，并在不需要过多空间时自动收缩。Java 堆所使用的内存不需要保证是连续的
>
